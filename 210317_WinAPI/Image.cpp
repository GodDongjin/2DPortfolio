...
void Image::Render(HDC hdc, int destX, int destY, bool isCenterRenderring, FPOINT size)
{
    int x = destX;
    int y = destY;
    ...
    if (isTransparent)
    {
        ...
        GdiTransparentBlt(
            hdc,
            x, y,
            imageInfo->width + size.x, imageInfo->height + size.y,
    ...
    else
    {
        ...
        BitBlt(
            hdc,                // 복사 목적지 DC
            x, y,               // 복사 시작 위치
            imageInfo->width + size.x,   // 원본에서 복사될 가로크기
            imageInfo->height + size.y,  // 원본에서 복사될 세로크기
...
void Image::FrameRender(HDC hdc, int destX, int destY,
    float currFrameX, float currFrameY, bool isCenterRenderring, float size)
{
...
void Image::UiBarRender(HDC hdc, int destX, int destY, bool isCenterRenderring, float maxPer, float nowPer)
{
    int x = destX;
    int y = destY;
    if (isCenterRenderring)
